...
export function CardCloseOpen() {
  const [status, setStatus] = useState(
    () =>
      JSON.parse(localStorage.getItem("status") || "{}")?.status || false
  );
  ...
  function ToggleStatus() {
    setStatus(!status)
    const date = new Date();
    localStorage.setItem("status", JSON.stringify({status: !status, time: date.toISOString()}));
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="flex items-center justify-between">
        <span
          className={`text-sm font-semibold ml-2 text-gray-500 ${
            !status && "text-red-500"
          }`}
        >
          Fechado
        ...
        <label htmlFor="toggle" className="flex items-center cursor-pointer">
          <div className="relative">
            ...
            <div
              onClick={ToggleStatus}
              className={`absolute left-0.5 top-0.5 bg-white w-4 h-4 rounded-full transition ease-in-out transform ${
